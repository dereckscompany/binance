...
- `get_exchange_info()` (spot and futures) - one row per symbol. The full per-symbol `filters` array is preserved as a JSON-encoded `filters_raw` column alongside the curated `lot_*` / `price_*` / `min_notional` columns (recover with `jsonlite::fromJSON(dt$filters_raw[1])`). Exchange-wide metadata (`timezone`, `server_time`, `rate_limits`, `exchange_filters`, and the variant-specific extras — `sors` for spot, `futures_type` + `assets` for futures) is attached as attributes: `attr(dt, "rate_limits")` etc.
