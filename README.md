...
withdrawals, transfers and sub-accounts – all from R scripts. Requests
can run the ordinary way, where your code waits for each answer, or in
the background so your program keeps working and collects the results
later. For research it can bulk-download years of price history and
resume where it left off if it is interrupted, handing everything back
as tidy tables ready for analysis. Because it can move real funds and
place real trades, it is built to be driven deliberately and tested with
small amounts first.
