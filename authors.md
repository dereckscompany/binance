# Authors and Citation

## Authors

- **[Dereck Mezquita](https://derecksnotes.com)**. Author, maintainer.
  [](https://orcid.org/0000-0002-9307-6762)

## Citation

Source:
[`inst/CITATION`](https://github.com/dereckscompany/binance/blob/chore/data-shapes-execute/inst/CITATION)

Mezquita, D. (2026). binance: R API Wrapper to Binance Cryptocurrency
Exchange. R package version 0.0.1.

    @Manual{,
      title = {binance: R API Wrapper to Binance Cryptocurrency Exchange},
      author = {Dereck Mezquita},
      year = {2026},
      note = {R package version 0.0.1},
    }
